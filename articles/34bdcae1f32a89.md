...
published: false
---
マネジメントコンソールからは簡単に実施できるLambda＠Edgeのデプロイですが、CLIで自動化するスクリプトを書くのはやや面倒です。本記事ではGitHub ActionsからLambda@EdgeへデプロイするWorkflowを紹介します。ご参考になれば幸いです。
