---
title: "GitHub ActionsからLambda@Edgeへデプロイする"
emoji: "🦭"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["GitHubActions", "AWS", "Lambda", "CloudFront"]
published: false
---
本記事ではGitHub ActionsからLambda@EdgeへデプロイするWorkflowを紹介します。Lambda＠Edgeへのデプロイ自動化のご参考になれば幸いです。
...
- Lambda＠Edgeへデプロイした後に問題が発生したとき、CloudFrontに関連づけるLambda@Edgeのバージョンを戻しやすい。Lambda関数の更新まで切り戻さなくて済む。
- Workflow実行直後に関数コードの問題に気づく可能性がある。Workflowを分けておけば、問題のある関数がそのままLambda@Edgeへデプロイされることを防げる。

## Workflowを手動で実行してもらう理由
`workflow_dispatch`限定のWorkflowにしました。ブランチをチェックアウト（スイッチ）する手間が省けるメリットがあると考えています。

# Workflow
```yml:update-lambda.yml
    ...
    steps:
      ...
      - name: Set constants for develop
        ...
        run: |
          echo "AWS_ROLE_ARN=aws:iam::your-aws-account-number:role/your-iam-role-arn-name" >> $GITHUB_ENV
          echo "AWS_LAMBDA_FUNCTION_ARN=arn:aws:lambda:us-east-1:your-aws-account-number:function:your-lambda-function-name" >> $GITHUB_ENV
          echo "AWS_CLOUDFRONT_DISTRIBUTION_ID=arn:aws:cloudfront::your-aws-account:distribution/your-cloudfront-distribution-id" >> $GITHUB_ENV
...
改行を含む場合、改行ありの値を参照するには[複数行の文字列](https://docs.github.com/ja/actions/using-workflows/workflow-commands-for-github-actions#multiline-strings)に従って書く必要があります。
...
```
改行を含む場合は前述の複数行の文字列を代入する場合の書き方に従わないといけません。しかし、CloudFrontのupdate-distributionコマンドのオプションに渡すJSONファイルは改行ありの必要はありませんので、ダブルクォーテーションで囲わずにあえて`echo key=value >> $GITHUB_ENV`と書いています。そのためCURRENT_CONFIGには一行の長いJSONが入っています。
...
# IAM
GitHub Actionsが引き受けるIAM Roleに割り当てるIAM Policyは以下になりました。
